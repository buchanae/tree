...
function Version10() {
  ...
  var geometries = {
    cylinder: new THREE.CylinderGeometry(10, 10, 10, 5, 4, false),
    sphere: new THREE.SphereGeometry(10, 10, 10),
    box: new THREE.BoxGeometry(30, 30, 30, 1, 1, 1),
  }
  ...
  for (var i = 0; i < opts.levels.length; i++) {
    (function(o) {
      ...
      g.add(o, "trend", -0.1, 0.1).step(0.01).onChange(ctrl.refresh);
      ...
      g.add(o, "minBranchIndex", 1, 200).step(1).onChange(ctrl.refresh);
...
function Node(depth, opts) {
  var geo = geometries[opts.geometry];
  return {
    index: 0,
    depth: depth,
    age: 1,
    ...
    leaves: [],
    bone: new THREE.Mesh(geo, opts.material),
  };
}
...
function GrowBranch(branch, year, opts) {
  ...
  newNode.bone.scale.set(1.0 - lopts.decay, 1.0, 1.0 - lopts.decay);
