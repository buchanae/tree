...
function Version4() {
  ...
  function makeBranch(offset, rotation, trunk) {
    ...
    var branchLength = (1 - offset) * maxBranchLength;
    ...
    var startRadius = trunk.radiusCurve.getPoint(offset).y;

    var startPosition = trunk.path.getPoint(offset);
    var endPosition = startPosition.clone().add(direction.setLength(branchLength));
  ...
  for (var i = 0; i < 30; i++) {
    ...
    var branchGeometry = makeBranch(offset, rotation, trunk);
    geometry.merge(branchGeometry);
    offset += 0.02;
    rotation += 1.1;
  }
